...
import { 
  ...
  Camera, X, AlertTriangle, Droplet,
  ClipboardList, Maximize2, Loader2, MapPin, Edit2, FileText
} from 'lucide-react';
import { 
  Patient, Lesion, Assessment, ExudateLevel, ExudateType, 
  TissuePercentage, LesionType, User
} from '../types';
import { getTreatmentSuggestion } from '../services/firebaseGeminiService';
...
const PatientDetail: React.FC<PatientDetailProps> = ({ user }) => {
  ...
  // State for new assessment form visibility
  ...
  const handleAddAssessment = async (assessmentData: Partial<Assessment>) => {
    if (!activeLesion) return;

    const assessment: Assessment = {
        id: Date.now().toString(),
        date: new Date().toISOString(),
        widthMm: assessmentData.widthMm || 0,
        heightMm: assessmentData.heightMm || 0,
        depthMm: assessmentData.depthMm || 0,
        tunnelingMm: assessmentData.tunnelingMm || 0,
        exudate: assessmentData.exudate || ExudateLevel.NONE,
        exudateType: assessmentData.exudateType,
        tissueTypes: assessmentData.tissueTypes || { necrotic: 0, slough: 0, granulation: 0, epithelialization: 0},
        infectionSigns: assessmentData.infectionSigns || [],
        woundEdges: assessmentData.woundEdges || [],
        periwoundSkin: assessmentData.periwoundSkin || [],
        painLevel: assessmentData.painLevel || 0,
        notes: assessmentData.notes || '',
        imageUrl: assessmentData.imageUrl,
        aiSuggestion: assessmentData.aiSuggestion
    };
