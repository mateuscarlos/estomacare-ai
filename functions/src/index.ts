...
if (!GEMINI_API_KEY) {
  console.error('GEMINI_API_KEY not configured in Cloud Functions environment');
}
 ...
 * }
 */
export const getTreatmentSuggestion = functions.https.onCall(async (data, context) => {
  ...
  const { lesion, currentAssessment, patientInfo } = data;
  ...
  try {
    ...
    console.log(`Treatment suggestion generated for user ${context.auth.uid}`);
 ...
 * }
 */
export const analyzeWoundImage = functions.https.onCall(async (data, context) => {
  // Verify user is authenticated
  if (!context.auth) {
    throw new functions.https.HttpsError(
      'unauthenticated',
  ...
  try {
    ...
    // Log usage for monitoring
    console.log(`Image analyzed for user ${context.auth.uid}`);
