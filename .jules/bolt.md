...
## 2024-05-24 - [React Memoization Broken by Inline Props]
**Learning:** In `Dashboard.tsx`, `StatCard` was memoized but received a new `icon` element (e.g. `<Icon />`) on every render, defeating memoization and causing unnecessary re-renders.
**Action:** Refactored `StatCard` to accept the component type (e.g. `Icon={Users}`) instead of an element. This ensures the prop reference remains stable across renders.
